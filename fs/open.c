...
PRIVATE struct inode * new_inode_dir(int dev, int inode_nr, int start_sect);
PRIVATE struct inode * create_directory(char * path, int flags);
void tempsearch(char * path);
void printallchildfiles(struct inode * dir_inode,int ifprint);
...
PUBLIC int do_open_old()
{
	...
	if (flags & O_CREAT) {
		if (inode_nr) {
			...
			if(filetype)
			{
				//printl("create directory\n");
				pin = create_directory(pathname, flags);
			}
			else
			{	
				//printl("create file\n");
	...
	if (pin) {
		...
		printl("after open %s cnt is %d",pathname,pin->i_cnt);
		...
		if (imode == I_CHAR_SPECIAL) {
			...
			printallchildfiles(pin,0);
			printallchildfiles(pin,1);
			assert(pin->i_mode == I_DIRECTORY);
			pin->i_cnt=1;
		}
		else {
...
PUBLIC int do_open()
{
	...
	int filetype=fs_msg.FILE_TYPE;	/* caller proc nr. */
	assert(name_len < MAX_PATH);
	phys_copy((void*)va2la(TASK_FS, pathname),
		  (void*)va2la(src, fs_msg.PATHNAME),
		  name_len);
	...
	if(pathname[0]==0)
	{
		printallchildfiles(root_inode,0);
		printallchildfiles(root_inode,1);
		assert(root_inode->i_mode == I_DIRECTORY);
		return -3;
	}
...
PUBLIC int do_cd()  //same as do_open
{
	int fd = -1;		/* return value */
 ...
 *
 * @param[in] path   The full path of the new directory
 * @param[in] flags  Attribiutes of the new file
 *
 * @return           Ptr to i-node of the new directory if successful, otherwise 0.
 * 
 * @see open()
 * @see do_open()
 *****************************************************************************/
PRIVATE struct inode * create_directory(char * path, int flags)
{
	...
	struct inode * dir_inode;
	if (strip_path_dir(directoryname, path, &dir_inode) != 0)
		return 0;

	int inode_nr = alloc_imap_bit(dir_inode->i_dev);
	int free_sect_nr = alloc_smap_bit(dir_inode->i_dev,
					  NR_DEFAULT_FILE_SECTS);
	struct inode *newino = new_inode_dir(dir_inode->i_dev, inode_nr,
					 free_sect_nr);

	new_dir_entry(dir_inode, newino->i_num, directoryname);
	printl("create dir ok");
	return newino;
}
...
void printallchildfiles(struct inode * dir_inode,int ifprint)
{
	...
	for (int i = 0; i < nr_dir_blks; i++) {
		...
		for (int j = 0; j < SECTOR_SIZE / DIR_ENTRY_SIZE; j++,pde++) {
			temp=get_inode(dir_inode->i_dev,pde->inode_nr);
			if (temp->i_mode == I_REGULAR)
			{
				if(ifprint)
					printl("file:%s \n",pde->name);

			}
			else if(temp->i_mode==I_DIRECTORY)
			{
				if(ifprint)
					printl("dir:%s \n",pde->name);
			}
